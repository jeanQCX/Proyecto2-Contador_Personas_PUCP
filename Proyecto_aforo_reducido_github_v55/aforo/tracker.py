...
# ---------------------------------------------------------------------------
MODELO_PATH = "best_640_ncnn_model"   # carpeta del modelo NCNN fine-tuned
#TRACKER_CONFIG = "bytetrack.yaml"
TRACKER_CONFIG = "my_bytetrack.yaml"   #yaml personalizado
...
class Tracker:
    ...
    def get_fps(self) -> float:
        """
        Devuelve el FPS del ultimo trackear() ejecutado, calculado solo
        sobre el tiempo de inferencia (model.track()), sin contar captura
        de camara ni el resto del pipeline.

        Antes este valor solo se usaba internamente para dibujarlo en
        debug_frame(). Este getter lo expone como numero, para que
        cualquier consumidor (ej: main.py, que no dibuja nada) pueda
        usarlo sin necesitar la ventana de OpenCV.
        """
        return self._fps
