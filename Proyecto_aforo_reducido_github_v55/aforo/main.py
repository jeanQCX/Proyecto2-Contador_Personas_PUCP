...
class AforoEngine:
    ...
    Configuracion UART leida de config.json bajo la clave "uart":
        puerto    -- puerto serie, ej: "/dev/ttyAMA0"
    ...
    def __init__(self):
        ...
        self._uart_puerto = uart_cfg.get("puerto",    "/dev/ttyAMA0")
        ...
        # Variables para calcular el FPS del sistema completo (todo el loop:
        # captura + tracker + counter, sin contar el tiempo de espera de
        # vuelta a leer un frame nuevo). Se actualiza cada iteracion de run(),
        # pero solo se IMPRIME en el momento del envio UART, para no saturar
        # la terminal con un print por frame.
        self._t_frame_anterior = time.time()
        self._fps_sistema      = 0.0
    ...
    def run(self):
        ...
        while self._activo:
            ...
            # FPS del sistema completo: tiempo entre el inicio de este frame
            # y el inicio del anterior. Se actualiza cada iteracion para que
            # el valor este fresco al momento de imprimirlo, pero el print
            # en si solo ocurre cuando toca enviar UART (mas abajo), no en
            # cada frame, para no saturar la terminal.
            t_now = time.time()
            dt_frame = t_now - self._t_frame_anterior
            self._fps_sistema      = 1.0 / dt_frame if dt_frame > 0 else 0.0
            self._t_frame_anterior = t_now
            ...
            if t_now - self._t_ultimo_envio >= self._intervalo:
                ...
                self._imprimir_log_terminal(conteos)
                self._t_ultimo_envio = t_now

        self._cerrar()

    def _imprimir_log_terminal(self, conteos: dict):
        """
        Imprime en terminal, solo para diagnostico, la misma informacion
        que se acaba de mandar por UART real, mas el FPS del tracker
        (modelo YOLO + ByteTrack) y el FPS del sistema completo (loop
        entero: captura + tracker + counter).

        Esto NO afecta ni modifica la trama UART real, que sigue
        mandando exactamente los mismos 6 valores de siempre. Es
        unicamente informativo en consola, en el mismo momento que
        ocurre el envio real, para que sea facil correlacionar ambos.
        """
        trama = (
            f"#{conteos['p_acu_in']},"
            f"{conteos['p_acu_out']},"
        ...
        fps_tracker = self._tracker.get_fps()
        print(f"  [UART TX] {trama}  |  "
              f"FPS tracker: {fps_tracker:5.1f}  "
              f"FPS sistema: {self._fps_sistema:5.1f}")
