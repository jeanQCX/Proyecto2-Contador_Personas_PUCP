...
FUENTE = "noche1.mp4"   # path a video
#FUENTE = None                 # None = camara

# ---------------------------------------------------------------------------
# Configuracion UART debug - editar aqui directamente
# ---------------------------------------------------------------------------
UART_ACTIVO    = True             # True = manda por serial, False = solo imprime
UART_PUERTO    = "/dev/ttyAMA0"   # puerto serie hacia el ESP32
...
COLOR_EPSILON_PERSONAS  = (50,  255, 50 )   # verde lima  - legacy, epsilon ya no se dibuja
COLOR_EPSILON_VEHICULOS = (255, 200, 100)   # azul claro  - legacy, epsilon ya no se dibuja
COLOR_CENTROIDE_FILTRADO = (0,  255, 255)   # amarillo - punto filtrado (Kalman/EMA)
...
def dibujar_linea(vis, linea, color, color_epsilon=None, epsilon_px=0):
    """
    Dibuja la linea de referencia que configuro el usuario.

    Se dibuja PUNTEADA porque ya no es la linea que confirma el cruce:
    con METODO_CONFIRMACION = "n_frames" o "ninguno" si cuenta el cruce
    real, pero con "doble_linea" es solo una referencia visual de donde
    esta el punto medio entre las dos lineas de confirmacion solidas
    (ver dibujar_lineas_confirmacion).

    Los parametros color_epsilon y epsilon_px se mantienen por
    compatibilidad de firma pero ya no se usan: el epsilon es legacy
    (deberia estar en 0.0 en config.json) y ya no tiene rol funcional,
    asi que tampoco se dibuja para no confundir visualmente.
    """
    if linea is None:
        return
    p1, p2, p3 = linea
    ...
    # Linea de referencia del usuario, punteada
    _linea_punteada(vis, pt1, pt2, color, grosor=2, segmento=10, espacio=7)
...
def dibujar_lineas_confirmacion(vis, lineas_confirmacion, color):
    """
    Dibuja las dos lineas SOLIDAS de confirmacion del metodo "doble_linea":
    linea_in y linea_out, generadas por counter.get_lineas_confirmacion().

    A diferencia de la linea de referencia del usuario (punteada), estas
    son las que realmente determinan cuando se confirma un cruce, por eso
    se dibujan solidas, como las lineas de conteo se dibujaban antes.

    Parametros:
        lineas_confirmacion : (linea_in, linea_out) o None si el metodo
                               activo no es "doble_linea" o la linea base
                               no esta configurada
        color                : color base del tipo (persona o vehiculo)
    """
    if lineas_confirmacion is None:
        return

    linea_in, linea_out = lineas_confirmacion

    for linea, etiqueta in [(linea_in, "IN"), (linea_out, "OUT")]:
        p1, p2, p3 = linea
        pt1 = (int(p1[0]), int(p1[1]))
        pt2 = (int(p2[0]), int(p2[1]))
        ...
        # Etiqueta en el punto medio de la linea para distinguir cual es cual
        mx = int((pt1[0] + pt2[0]) / 2)
        my = int((pt1[1] + pt2[1]) / 2)
        cv2.putText(vis, etiqueta, (mx + 6, my - 6),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.45, color, 1)

def dibujar_centroides_filtrados(vis, objetos, centroides_filtrados):
    """
    Dibuja un circulo amarillo en la posicion filtrada (Kalman o EMA) de
    cada objeto. Permite comparar visualmente contra el punto rojo crudo
    (ya dibujado por tracker.debug_frame) sin elementos extra que distraigan.

    Si un track_id no esta en centroides_filtrados (FILTRO="ninguno"),
    no se dibuja nada para ese ID: el punto filtrado coincide con el
    crudo, entonces no hay diferencia que mostrar.

    Parametros:
        vis                   : frame sobre el que se dibuja (ya tiene el rojo)
        objetos               : lista de tracker.trackear() de este frame
        centroides_filtrados  : dict {track_id: (x, y)} de counter.get_centroides_filtrados()
    """
    for obj in objetos:
        track_id = obj["track_id"]
        if track_id not in centroides_filtrados:
            continue  # FILTRO="ninguno", no hay punto filtrado que dibujar

        fx, fy = centroides_filtrados[track_id]
        fx, fy = int(fx), int(fy)

        # Punto filtrado en amarillo
        cv2.circle(vis, (fx, fy), 5, COLOR_CENTROIDE_FILTRADO, -1)
        cv2.putText(vis, "F", (fx + 7, fy + 4),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.4, COLOR_CENTROIDE_FILTRADO, 1)
...
def main():
    ...
    while True:
        ...
        centroides_filtrados  = counter.get_centroides_filtrados()
        lineas_confirmacion   = counter.get_lineas_confirmacion()
        ...
        # Capas adicionales
        dibujar_roi(vis, roi)
        # Linea de referencia del usuario, punteada (ver dibujar_linea)
        dibujar_linea(vis, linea_personas,  COLOR_LINEA_PERSONAS)
        dibujar_linea(vis, linea_vehiculos, COLOR_LINEA_VEHICULOS)
        # Lineas solidas de confirmacion, solo aparecen con METODO_CONFIRMACION="doble_linea"
        dibujar_lineas_confirmacion(vis, lineas_confirmacion["persona"],  COLOR_LINEA_PERSONAS)
        dibujar_lineas_confirmacion(vis, lineas_confirmacion["vehiculo"], COLOR_LINEA_VEHICULOS)
        dibujar_centroides_filtrados(vis, objetos, centroides_filtrados)
